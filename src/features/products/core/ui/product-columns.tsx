...
export function productColumns({
  ...
  return [
    ...
    },
    {
      accessorKey: "stock",
      header: ({ column }) => (
        <SortableHeader column={column} title="In Stock" />
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: ({ column }) => (
        <SortableHeader column={column} title="Variants" />
      ),
      cell: ({ row }) => (
        <div className="tabular-nums text-center">
    ...
    // Only include actions column if user has delete permission
    ...(canDelete
      ? [
          {
            accessorKey: "actions",
            header: () => <div className="text-center">Action</div>,
            cell: ({ row }: { row: Row<ProductListRow> }) => (
              <DeleteIconDialog
                endpoint={`/api/catalog/products/${row.original.id}`}
                refetchKey="products"
                useIconButton
              />
            ),
            enableSorting: false,
          } satisfies ColumnDef<ProductListRow>,
        ]
      : []),
  ];
}
